...
test("config exposes core + extra courses", () => {
  ...
  // Extras (hidden until filtered): 15-min solo sessions, Taster, Jam, Drop-in, clinics.
  assert.deepEqual(extra.map(c => c.id), [713, 714, 9, 478, 586, 67, 78, 66]);
...
test("filter row 1 is the four ride sessions, row 2 the rest", () => {
  ...
  assert.deepEqual(row("other"), ["Taster", "Jam", "Drop-in", "Skills", "Tantrums", "Clinic"]);
});
